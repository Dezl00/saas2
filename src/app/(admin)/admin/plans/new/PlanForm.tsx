        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" name="name" defaultValue={initialData?.name} required className="w-full px-4 py-2.5 bg-surface-50 border border-surface-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" placeholder="مثال: الباقة الاحترافية" />
            </div>
            <div>
              <label className="block text-sm font-medium text-surface-900 mb-1">السعر (ج.م) *</label>
              <input type="number" step="0.01" name="price" defaultValue={initialData?.price} required className="w-full px-4 py-2.5 bg-surface-50 border border-surface-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" placeholder="0.00" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-surface-900 mb-1">وصف الباقة</label>
              <input type="text" name="description" defaultValue={initialData?.description} className="w-full px-4 py-2.5 bg-surface-50 border border-surface-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" placeholder="وصف قصير يظهر للعملاء..." />
            </div>
            <div>
              <label className="block text-sm font-medium text-surface-900 mb-1">المدة (بالأيام) *</label>
              <input type="number" name="durationDays" defaultValue={initialData?.durationDays ?? "30"} required className="w-full px-4 py-2.5 bg-surface-50 border border-surface-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-surface-900 mb-1">الترتيب</label>
              <input type="number" name="sortOrder" defaultValue={initialData?.sortOrder ?? "0"} className="w-full px-4 py-2.5 bg-surface-50 border border-surface-200 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" />
            </div>
          </div>
        </div>
