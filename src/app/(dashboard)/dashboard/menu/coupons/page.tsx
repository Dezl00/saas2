        ...
        <div className="bg-surface-50 border-2 border-surface-100 rounded-[32px] p-6 lg:p-8 self-start sticky top-24 max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-hide">
          ...
          <form action={addCoupon as any} className="space-y-4">
            <div>
              ...
              <input type="text" name="code" id="code" required placeholder="مثال: SUMMER20" dir="ltr" className="w-full px-4 py-3 bg-white border-2 border-surface-200 rounded-2xl text-surface-950 focus:border-primary-500 outline-none uppercase font-black tracking-wider transition-colors" />
            </div>
            
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="type" className="block text-sm font-bold text-surface-950 mb-2">نوع الخصم *</label>
                <select name="type" id="type" className="w-full px-4 py-3 bg-white border-2 border-surface-200 rounded-2xl text-surface-950 focus:border-primary-500 outline-none font-medium transition-colors cursor-pointer">
              ...
              <div>
                ...
                <input type="number" name="value" id="value" required min="1" step="0.1" placeholder="مثال: 20" dir="ltr" className="w-full px-4 py-3 bg-white border-2 border-surface-200 rounded-2xl text-surface-950 focus:border-primary-500 outline-none font-bold transition-colors text-left" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" name="minOrder" id="minOrder" min="0" step="0.5" placeholder="مثال: 100" dir="ltr" className="w-full px-4 py-3 bg-white border-2 border-surface-200 rounded-2xl text-surface-950 focus:border-primary-500 outline-none font-bold transition-colors text-left" />
              </div>
              <div>
                <label htmlFor="maxDiscount" className="block text-sm font-bold text-surface-950 mb-2">الحد الأقصى للخصم</label>
                <input type="number" name="maxDiscount" id="maxDiscount" min="0" step="0.5" placeholder="مثال: 50" dir="ltr" className="w-full px-4 py-3 bg-white border-2 border-surface-200 rounded-2xl text-surface-950 focus:border-primary-500 outline-none font-bold transition-colors text-left" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" name="usageLimit" id="usageLimit" min="1" placeholder="مثال: 100" dir="ltr" className="w-full px-4 py-3 bg-white border-2 border-surface-200 rounded-2xl text-surface-950 focus:border-primary-500 outline-none font-bold transition-colors text-left" />
              </div>
              <div>
                <label htmlFor="expiresAt" className="block text-sm font-bold text-surface-950 mb-2">تاريخ الانتهاء</label>
                <input type="date" name="expiresAt" id="expiresAt" className="w-full px-4 py-3 bg-white border-2 border-surface-200 rounded-2xl text-surface-950 focus:border-primary-500 outline-none font-bold transition-colors cursor-pointer" />
              </div>
            </div>

            <button type="submit" className="w-full mt-2 py-4 bg-primary-600 hover:bg-primary-700 text-white rounded-2xl font-bold transition-all flex items-center justify-center gap-2 text-lg">
              ...
              return (
                <div key={coupon.id} className={`bg-white border-2 rounded-[24px] p-5 lg:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 transition-colors ${isInvalid ? 'opacity-60 border-surface-100 bg-surface-50' : 'border-surface-100 hover:border-surface-200'}`}>
                  <div className="flex items-center gap-5">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border-2 ${coupon.type === 'PERCENTAGE' ? 'bg-primary-50 text-primary-600 border-primary-100' : 'bg-accent-50 text-accent-600 border-accent-100'}`}>
                    ...
                    <div>
                      <div className="flex items-center gap-3">
                        ...
                        {isExpired && <span className="text-xs bg-error-50 text-error-700 border-2 border-error-100 px-3 py-1 rounded-xl font-bold">منتهي الصلاحية</span>}
                        {isFullyUsed && <span className="text-xs bg-error-50 text-error-700 border-2 border-error-100 px-3 py-1 rounded-xl font-bold">مستنفذ العدد</span>}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-3 text-sm text-surface-500 font-medium mt-3">
                        <span className="bg-surface-100 px-3 py-1 rounded-xl">الاستخدام: <strong className="text-surface-950">{coupon.usedCount} {coupon.usageLimit ? `/ ${coupon.usageLimit}` : 'مرة'}</strong></span>
                        {coupon.minOrder && <span className="bg-surface-100 px-3 py-1 rounded-xl">الحد الأدنى للطلب: <strong className="text-surface-950">{formatPrice(Number(coupon.minOrder), store?.currency)}</strong></span>}
                      </div>
                    </div>
                  </div>
