...
export function ClientOrderMasterDetail({ 
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-8rem)] md:h-[calc(100vh-6rem)] -m-4 md:-m-8 border-t border-surface-100 md:rounded-3xl overflow-hidden bg-white">
      ...
      <div className="md:hidden flex items-center justify-between p-4 border-b border-surface-100 bg-white">
        ...
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 bg-surface-50 text-surface-700 font-bold rounded-xl"
        >
      ...
      <div className={`flex-1 overflow-y-auto bg-white transition-opacity ${isPending ? 'opacity-70' : 'opacity-100'} p-4 md:p-8 lg:p-12`}>
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-surface-100 pb-6">
            ...
            <span className={`px-4 py-2 font-bold text-sm rounded-xl ${statusMap[optimisticStatus]?.color || 'bg-surface-50 text-surface-800'}`}>
          ...
          {selectedOrder.notes && (
            <div className="bg-[#FFF8E1] rounded-2xl p-4 text-[#F57F17] text-sm">
          ...
          <form action={handleStatusUpdate} className="pt-6 border-t border-surface-100">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <select name="status" defaultValue={optimisticStatus} className="flex-1 px-4 py-3 bg-surface-50 border-transparent rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none font-bold text-surface-900 transition-all cursor-pointer">
                  <option value="PENDING">قيد الانتظار</option>
                  <option value="CONFIRMED">مؤكد (جاري التحضير)</option>
                  <option value="DELIVERED">مكتمل (تم التسليم)</option>
                  <option value="CANCELLED">ملغي</option>
                </select>
                <button type="submit" disabled={isPending} className="px-6 py-3 bg-primary-600 hover:bg-primary-700 disabled:opacity-50 text-white font-bold rounded-xl transition-colors whitespace-nowrap">
                  حفظ الحالة
                </button>
              </div>
            </div>
