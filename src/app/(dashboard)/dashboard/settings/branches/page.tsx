      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border-2 border-surface-100 rounded-[32px] p-6 lg:p-8 self-start sticky top-24 max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-hide">
          ...
          <form action={addBranch as any} className="space-y-4">
            ...
            <div>
              <label htmlFor="whatsappNumber" className="block text-sm font-bold text-surface-950 mb-2">رقم الواتساب (للطلبات)</label>
              <input type="text" name="whatsappNumber" id="whatsappNumber" dir="ltr" placeholder="01xxxxxxxxx" className="w-full px-5 py-3.5 bg-white border border-surface-200 rounded-[24px] text-surface-950 font-medium focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors outline-none text-end" />
            </div>
            <div>
              <label htmlFor="mapUrl" className="block text-sm font-bold text-surface-950 mb-2">رابط خرائط جوجل</label>
              <input type="url" name="mapUrl" id="mapUrl" dir="ltr" placeholder="https://maps.google.com/..." className="w-full px-5 py-3.5 bg-white border border-surface-200 rounded-[24px] text-surface-950 font-medium focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors outline-none text-end" />
            </div>
            ...
            branches.map(branch => (
              <div key={branch.id} className="bg-white border-2 border-surface-100 rounded-[24px] p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-colors hover:border-surface-200">
                <div>
                  ...
                  {branch.phone && <p className="text-sm font-medium text-surface-500 mt-2" dir="ltr">هاتف: {branch.phone}</p>}
                  {branch.whatsappNumber && <p className="text-sm font-medium text-surface-500 mt-1" dir="ltr">واتساب: {branch.whatsappNumber}</p>}
                  {branch.address && <p className="text-sm font-medium text-surface-500 mt-1">{branch.address}</p>}
                  {branch.mapUrl && <a href={branch.mapUrl} target="_blank" rel="noreferrer" className="text-sm font-bold text-primary-600 mt-1 inline-block hover:underline">عرض الخريطة</a>}
                </div>
